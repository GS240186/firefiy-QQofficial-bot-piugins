...
async def handle(ctx) -> bool:
    """统一分发入口。返回 True 表示已处理，False 表示放行。仅群聊场景。"""
    if not ctx.is_group:
        return False
    from console_server import is_feature_enabled
    if not is_feature_enabled("group_admin", appid=ctx.bot_appid):
        return False
    return await ctx.bot._time_plugin(
        "group_admin", _manager.handle_command, ctx.perf,
        ctx.api, ctx.content, ctx.target_id, ctx.member_openid, ctx.msg_id,
        member_role=ctx.member_role, is_console_admin=ctx.is_console_admin,
    )
