...
class CheckInManager:
    ...
    def _get_user(self, data: dict, group_openid: str, member_openid: str) -> dict:
        ...
        user.setdefault("lottery_day", "")
        user.setdefault("lottery_count", 0)
        user.setdefault("lottery_rate", 0.0)
        return user
    ...
    async def do_lottery(self, api, group_openid: str, member_openid: str, msg_id: str,
                         scene=None, target_id: str = None, member_nick: str = ""):
        """消耗固定积分抽奖，按概率中奖返还随机积分；每日抽奖次数受 lottery_daily_limit 限制，中奖概率每天随机（30%~40%）。"""
        import random
        today = today_str()
        data = self._load_data(group_openid)
        user = self._get_user(data, group_openid, member_openid)
        ...
        # 跨天重置：每日抽奖计数清零，并随机生成本日中奖概率（30%~40%）
        if user.get("lottery_day", "") != today:
            user["lottery_day"] = today
            user["lottery_count"] = 0
            user["lottery_rate"] = round(random.uniform(0.30, 0.40), 4)

        cfg = get_checkin_config()
        cost = cfg.get("lottery_cost", 50)
        limit = max(0, int(cfg.get("lottery_daily_limit", 2)))

        # 每日抽奖次数限制（0 = 不限制）
        used = int(user.get("lottery_count", 0))
        if limit > 0 and used >= limit:
            reply = (
                "🎰 积分抽奖\n"
                "❌ 今日抽奖次数已用完（每日 %d 次，今日已抽 %d 次）\n"
                "明天再来吧～"
            ) % (limit, used)
        ...
        # 抽奖逻辑：使用本日随机概率中奖（30%~40%），中奖金额区间 [cost, cost*3] 随机
        won = random.random() < float(user.get("lottery_rate", 0.35))
        reward = 0
        if won:
            reward = random.randint(cost, cost * 3)

        # 写回：中奖则把奖励加回积分，并累加每日抽奖次数
        if won:
            user["points"] += reward
        user["lottery_count"] = used + 1
        self._save_data(data, group_openid)

        # 拼接「剩余次数」提示
        remain = (limit - user["lottery_count"]) if limit > 0 else None
        if remain is None:
            hint = "再来一次？"
        elif remain > 0:
            hint = "还能再抽 %d 次～" % remain
        else:
            hint = "今日抽奖次数已用完，明天再来～"
        ...
        if won:
            reply = (
                ...
                "💰 剩余积分：%d\n"
                "🍀 运气不错，%s"
            ) % (cost, reward, user.get("points", 0), hint)
        else:
            reply = (
                "🎰 抽奖结果\n"
                "💨 很遗憾未中奖\n"
...
async def handle(ctx) -> bool:
    ...
    if not is_feature_enabled("checkin", appid=ctx.bot_appid):
        return False
    return await ctx.bot._time_plugin(
        "checkin", _manager.handle_command, ctx.perf,
        ctx.api, ctx.content, ctx.storage_id, ctx.member_openid, ctx.msg_id,
        scene=ctx.scene, member_nick=ctx.member_nick,
    )
