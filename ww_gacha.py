...
def _render_pulls_image_safe(pool, results, pity):
    """同步版本：带异常护栏——失败返回 None（调用方回退到文本）。"""
    if not _HAS_PIL:
        return None
    try:
...
async def render_pulls_image_safe(pool, results, pity):
    """异步版本：把 CPU 密集的图片渲染放到线程池，避免阻塞事件循环。"""
    return await asyncio.to_thread(_render_pulls_image_safe, pool, results, pity)
...
async def _do_pull(ctx, count):
    if not _POOLS:
        await ctx.reply("❌ 鸣潮抽卡：未找到任何卡池预设。")
        return True
    # 先给即时反馈，避免用户以为没响应而重复发送
    await ctx.reply("🌊 正在抽卡，请稍候...")
    with _STATE_LOCK:
        state = _load_state()
        rec = _get_user_state(state, ctx.storage_id)
        pool = _POOLS.get(rec["current"])
    ...
    png = await render_pulls_image_safe(pool, results, pity)
    if png:
...
async def _send_image(ctx, image_bytes, content=""):
    """通过内置 _send_image_bytes 发送图片 bytes（场景无关）。
