...
触发指令：
  开题：脑筋急转弯 / 急转弯 / jizhuanwan / jzw
  作答：直接发答案文本；或前缀"答案" / "答"
  换题：fun_brainteaser 换一题
  看答案：fun_brainteaser 看答案
  结束：fun_brainteaser 结束
"""
import json
import logging
...
QTYPE = "brain"
LABEL = "脑筋急转弯"
API_LIST = "https://v.api.aa1.cn/api/jizhuanwan/index.php?type=txt"
API_TIMEOUT = 8

PLUGIN = {
    "key": "game_brain_teaser",
    "name": "脑筋急转弯",
    "priority": 200,
    "description": "发送「脑筋急转弯」随机抽取一条脑筋急转弯，输入答案即可作答（超时 120s）",
    "category": "game",
    "config_schema": [
        {"key": "braineaser_timeout_seconds", "type": "int", "default": 120, "label": "答题超时（秒）", "min": 30, "max": 600},
        {"key": "braineaser_show_hint", "type": "bool", "default": False, "label": "是否显示答案提示"},
        {"key": "braineaser_skip_open", "type": "bool", "default": False, "label": "跳过开放题"},
    ],
}

# 开题触发词
TRIGGERS = ("脑筋急转弯", "急转弯", "jizhuanwan", "jzw")
# 答案前缀：用户用 "答案 X" 形式主动给答案
ANSWER_PREFIXES = ("答案", "答")
    ...
    try:
        ...
        for it in items:
            if not isinstance(it, dict):
                continue
            q = (it.get("title") or it.get("question") or it.get("问题") or it.get("content") or "").strip()
            a = (it.get("answer") or it.get("答案") or it.get("ans") or "").strip()
            # 简单清洗
            import re
        ...
        logger.error("[fun_brainteaser] 拉取失败: %s" % e)
    return None
...
async def cmd_start(api, scene, chat_id, owner, msg_id=None, event_id=None):
    ...
    if not item:
        ...
        return
    started = qa_start(chat_id, QTYPE, owner, scene, api, LABEL,
    ...
    if not started:
        await qa_send(api, scene, chat_id,
                       ...
                       mention_openid=owner, msg_id=msg_id, event_id=event_id)
        return
    text = "🧠 <b>%s</b>\n\n<b>题目</b>：%s\n\n📝 直接输入你的答案（120 秒内有效，超时自动结束）" % (LABEL, item["question"])
    pairs = (
        ("换一个", "fun_brainteaser 换一题"),
        ("看答案", "fun_brainteaser 看答案"),
        ("结束", "fun_brainteaser 结束"),
    )
...
async def cmd_next(api, scene, chat_id, owner, msg_id=None, event_id=None):
    ...
    if not qa_is_active(chat_id, QTYPE):
        ...
        return
    qa_continue(chat_id, QTYPE, {"answer": item["answer"], "question": item["question"]})
    text = "🧠 <b>%s</b> · 已换题\n\n<b>新题</b>：%s\n\n📝 直接输入你的答案（120 秒内有效）" % (LABEL, item["question"])
    pairs = (
        ("换一个", "fun_brainteaser 换一题"),
        ("看答案", "fun_brainteaser 看答案"),
        ("结束", "fun_brainteaser 结束"),
...
def is_command(text):
    t = (text or "").strip()
    return t.startswith("fun_brainteaser ")


def parse_command(text):
    """'fun_brainteaser 换一题' -> 'next' / 'show' / 'end'"""
    t = (text or "").strip()
    if not t.startswith("fun_brainteaser "):
        return None
    arg = t[len("fun_brainteaser "):].strip()
    if arg in ("换一题", "换一个", "next"):
        return "next"
    if arg in ("看答案", "show", "答案"):
        return "show"
    if arg in ("结束", "end", "退出"):
        return "end"
...
async def handle(ctx):
    ...
    try:
        from modules.common import ChatScene as _CS
        scene = ctx.scene
        ...
        # 1. 走「fun_brainteaser 换一题/看答案/结束/开题」命令（来自按钮回调）
        ...
        if qa_is_active(chat_id, QTYPE):
            ...
            if text and qa_is_owner(chat_id, QTYPE, owner) and len(text) <= 50:
                # 排除明显是问句或长文本
                if not text.startswith("fun_") and not any(t in text for t in ("脑筋急转弯", "急转弯", "jizhuanwan", "jzw")):
        ...
        logger.error("[fun_brainteaser] handle 异常: %s" % e)
        return False
