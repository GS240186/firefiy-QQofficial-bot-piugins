...
PLUGIN = {
    ...
    "name": "视频·帅哥",
    "priority": 50,
    "description": "发送「帅哥视频」推送",
    "category": "video",
    "config_schema": [
        {"key": "video_shuaige_enable_cache", "type": "bool", "default": False, "label": "是否启用缓存"},
        {"key": "video_shuaige_cache_ttl", "type": "int", "default": 300, "label": "缓存时间（秒）", "min": 0, "max": 86400},
        {"key": "video_shuaige_max_retries", "type": "int", "default": 3, "label": "最大重试次数", "min": 0, "max": 10},
    ],
}
...
async def handle(ctx) -> bool:
    ...
    if not is_feature_enabled("video", appid=ctx.bot_appid):
        return False
    if not is_feature_enabled("video_shuaige", appid=ctx.bot_appid):
        return False
    content = clean_content(ctx.content or "").strip()
    if content != "帅哥视频":
        return False
    return await ctx.bot._time_plugin(
        "video_shuaige", _manager.send_video, ctx.perf,
        ctx.api, "帅哥视频", ctx.target_id, ctx.msg_id, scene=ctx.scene,
    )
