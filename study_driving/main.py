# -*- coding: utf-8 -*-
"""学习·驾考学习（study_driving）—— 学习系统细粒度插件"""
...
PLUGIN = {
    ...
    "name": "学习·驾考学习",
    "priority": 30,
    "description": "发送「驾考学习」开始答题",
    "category": "study",
    "config_schema": [
        {"key": "study_driving_enable_ai", "type": "bool", "default": False, "label": "是否启用 AI 出题"},
        {"key": "study_driving_mode", "type": "select", "default": "quiz", "label": "答题模式", "options": ["quiz", "flashcard", "exam"]},
        {"key": "study_driving_timeout", "type": "int", "default": 120, "label": "每题超时（秒）", "min": 10, "max": 600},
    ],
}
...
async def handle(ctx) -> bool:
    ...
    if not is_feature_enabled("study", appid=ctx.bot_appid):
        return False
    if not is_feature_enabled("study_driving", appid=ctx.bot_appid):
        return False
    ...
    if not text:
        return False
    QTYPE = "quiz_driving"
    exact_map = {"驾考答案": QTYPE, "驾考跳过": QTYPE, "驾考下一题": QTYPE}
    submit_map = {"驾考作答": QTYPE}
    ...
    entry_map = {"驾考": QTYPE, "驾考学习": QTYPE, "考驾照": QTYPE}
