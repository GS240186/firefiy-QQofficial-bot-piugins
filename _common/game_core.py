...
from modules.qa_common import (
    qa_start, qa_is_active, qa_end, qa_owner_openid,
    qa_touch, qa_send, qa_send_kb,
)
from modules.chess_common import chess_arm, chess_touch, chess_cancel
...
def _chess_load_nested(path):
    """加载棋局文件并迁移旧版单局结构 group->game 到 group->{id:game}。"""
    d = load_json(path)
    changed = False
    for g in list(d.keys()):
        v = d[g]
        if isinstance(v, dict) and "board" in v and "status" in v and ("mode" in v or "current_player" in v):
            d[g] = {"1": v}
            changed = True
    if changed:
        try:
...
def _chess_next_id(games, group):
    """计算某群下一局棋的编号（基于已有数字键最大值 +1）。"""
    grp = games.get(group, {})
    n = 0
    for k in grp.keys():
        if str(k).isdigit():
...
def _chess_find_by_player(games, group, openid):
    """在嵌套棋局中查找某用户作为玩家的进行中棋局，返回 (game_id, game)。"""
    grp = games.get(group, {})
    for gid, g in grp.items():
        ...
        if g.get("status") != "playing":
            continue
        if openid in g.get("players", {}).values():
            return int(gid), g
    return None, None


def _chess_find_waiting(games, group, gtype):
    """查找某群等待第二位玩家加入的棋局（pvp 一方尚未分配），返回 (game_id, game)。"""
    grp = games.get(group, {})
    for gid, g in grp.items():
        if not str(gid).isdigit():
            continue
        if g.get("status") not in ("playing", "waiting"):
            continue
...
async def _gomoku_timeout_cb(api, scene, chat_id, game_type, game_id, owner, label):
    """五子棋超时回调：把文件中的对应棋局置为 ended（含等待第二位玩家的 waiting 局）。"""
    games = _chess_load_nested(GOMOKU_DATA_FILE)
    g = games.get(chat_id, {}).get(str(game_id))
    if g and g.get("status") in ("playing", "waiting"):
        g["status"] = "ended"
        g["end_reason"] = "timeout"
        games.setdefault(chat_id, {})[str(game_id)] = g
        save_json(GOMOKU_DATA_FILE, games)


async def _xiangqi_timeout_cb(api, scene, chat_id, game_type, game_id, owner, label):
    """象棋超时回调：把文件中的对应棋局置为 ended（含等待第二位玩家的 waiting 局）。"""
    games = _chess_load_nested(XIANGQI_DATA_FILE)
    g = games.get(chat_id, {}).get(str(game_id))
    if g and g.get("status") in ("playing", "waiting"):
        g["status"] = "ended"
        g["end_reason"] = "timeout"
...
class GameManager:
    ...
    async def _send_board(self, api, group_openid, board, content, msg_id=None, last_move=None, scene=None):
        ...
        try:
            ...
            img_bytes = await asyncio.to_thread(self._board_to_image, board, last_move=last_move)
    ...
    async def _gomoku_start_ai(self, api, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        ...
        sc = _scene(scene)
        games = _chess_load_nested(GOMOKU_DATA_FILE)
        gid = _chess_next_id(games, group_openid)

        board = self._new_board()

        games.setdefault(group_openid, {})[str(gid)] = {
        ...
        save_json(GOMOKU_DATA_FILE, games)
        chess_arm(group_openid, "gomoku", gid, api, sc, member_openid, "五子棋", on_timeout=_gomoku_timeout_cb)

        self._xq_save_name(member_openid, member_nick)

        mention = "<@!%s> " % member_openid if sc == ChatScene.GROUP else ""
        msg = (
            "%s五子棋（编号%d）AI对战开始！\n" % (mention, gid) +
            "你执黑先行，发送“下棋 H8”落子\n"
            "（列 A-O，行 1-15）"
    ...
    async def _gomoku_select_color(self, api, group_openid, member_openid, color, msg_id, scene=None, member_nick=None):
        ...
        self._xq_save_name(member_openid, member_nick)
        mention = "<@!%s> " % member_openid if sc == ChatScene.GROUP else ""
        msg = "%s二人对战（编号%d）开始！你执%s\n%s" % (mention, gid, color_name, turn_msg)
        await self._send_board(api, group_openid, board, msg, msg_id=msg_id, scene=scene)



    async def _gomoku_move(self, api, text, group_openid, member_openid, msg_id, game_id, scene=None, member_nick=None):

        """处理落子指令"""

        games = _chess_load_nested(GOMOKU_DATA_FILE)
        game = games.get(group_openid, {}).get(str(game_id))
        ...
        chess_touch(group_openid, "gomoku", game_id)
        ...
        if self._check_win(board, row, col, current_player):
            ...
            save_json(GOMOKU_DATA_FILE, games)
            chess_cancel(group_openid, "gomoku", game_id)
        ...
        if mode == "ai" and game["current_player"] == WHITE:
            ...
            mention = "<@!%s> " % human if (sc == ChatScene.GROUP and human) else ""
            msg = "%sAI落子 %s，轮到你落子" % (mention, ai_move_str)
        ...
        next_key = "white" if game["current_player"] == WHITE else "black"
        next_openid = players.get(next_key)
        mention = "<@!%s> " % next_openid if (sc == ChatScene.GROUP and next_openid) else ""
        msg = "%s%s落子 %s，轮到%s" % (mention, color_name, move_str, next_color)

        await self._send_board(api, group_openid, board, msg, msg_id=msg_id, last_move=(row, col), scene=scene)



    async def _gomoku_surrender(self, api, group_openid, member_openid, msg_id, game_id, scene=None, member_nick=None):
    ...
    async def _idiom_send_question(self, api, group_openid, idiom_item, round_num, msg_id, scene=None):
        ...
        # 玩家输入“猜成语作答 XXX”由“猜成语作答”按钮点击后填入输入框补齐：例如“猜成语作答 守株待兔”

        # 第1行：🔤 猜成语作答（type=2 指令按钮，enter=False）

        # 点击后输入框插入“猜成语作答”，玩家直接接上答案发送，如“猜成语作答 画蛇添足”

        row1 = [

            {"label": "🔤 猜成语作答", "command": "猜成语作答", "enter": False, "id": "idiom_answer_btn"},
    ...
    async def _idiom_start(self, api, group_openid, member_openid, msg_id, scene=None):
        ...
        # 进行中守卫：同一群/私聊同时只一个猜成语，避免按钮混淆
        if qa_is_active(group_openid, "idiom"):
            owner = qa_owner_openid(group_openid, "idiom")
            await qa_send(
                api, sc, group_openid,
                "🎯 猜成语正在进行中，请等待当前游戏结束后再开始～",
                mention_openid=owner,
            )
            return
        ...
        # 注册问答会话：2 分钟未操作自动结束，并清理文件棋局
        qa_start(
            group_openid, "idiom", owner=member_openid, scene=sc, api=api,
            label="猜成语", data={}, on_timeout=self._idiom_timeout_cb,
        )
    ...
    async def _idiom_timeout_cb(self, api, scene, chat_id, label, owner):
        """qa_common 超时回调：清理猜成语文件棋局（会话本体由 qa_common 自动 pop）。"""
        try:
            games = load_json(IDIOM_GAME_DATA_FILE)
            if chat_id in games and games[chat_id].get("status") == "playing":
                games[chat_id]["status"] = "ended"
                games[chat_id]["end_reason"] = "timeout"
                save_json(IDIOM_GAME_DATA_FILE, games)
        except Exception as e:
            logger.error("[猜成语] 超时清理失败: %s" % e)
    ...
    async def _idiom_correct(self, api, group_openid, member_openid, msg_id, scene=None):
        ...
        if current_round >= total_rounds:
            ...
            await qa_send(api, _scene(scene), group_openid,

                "✅ 回答正确！%s\n\n🎉 已完成全部 %d 轮，游戏结束！" % (current_idiom, total_rounds),

                mention_openid=member_openid, msg_id=msg_id,

            )

            qa_end(group_openid, "idiom")
    ...
    async def _idiom_wrong(self, api, group_openid, member_openid, guess, msg_id, scene=None):
    ...
    async def _xq_send_board(self, api, group_openid, g, msg_id=None, start=False, scene=None, game_id=None):

        board = g["board"]
        view = g["human_color"] if g["mode"] == "ai" else g["turn"]
        img = await asyncio.to_thread(self._xq_board_to_image, board, g.get("last_move"), view_color=view)
        ...
        if start:
            ...
            content = "🎉 象棋开局！（编号%d）\n" % (game_id if game_id is not None else 0) + content
    ...
    async def _xiangqi_start_ai(self, api, group_openid, member_openid, msg_id, human_color, scene=None, member_nick=None):
        ...
        games.setdefault(group_openid, {})[str(gid)] = g

        save_json(XIANGQI_DATA_FILE, games)
        chess_arm(group_openid, "xiangqi", gid, api, sc, member_openid, "中国象棋", on_timeout=_xiangqi_timeout_cb)
        ...
        await self._xq_send_board(api, group_openid, g, msg_id=msg_id, start=True, scene=scene, game_id=gid)
    ...
    async def _xiangqi_pvp_join(self, api, group_openid, member_openid, msg_id, color, scene=None, member_nick=None):
        ...
        sc = _scene(scene)
        games = _chess_load_nested(XIANGQI_DATA_FILE)
        gid, g = _chess_find_waiting(games, group_openid, "xiangqi")
        ...
        await send_text(api, _scene(scene), group_openid,
                              ...
                              "✅ 已就位（%s，编号%d）。等待另一名玩家发送「象棋双人」加入对局…"

                              % ("红方" if color == "red" else "黑方", gid), msg_id=msg_id)



    async def _xiangqi_move(self, api, text, group_openid, member_openid, msg_id, game_id, scene=None):

        games = _chess_load_nested(XIANGQI_DATA_FILE)
        g = games.get(group_openid, {}).get(str(game_id))

        if not g or g.get("status") != "playing":

            return

        chess_touch(group_openid, "xiangqi", game_id)
        sc = _scene(scene)
        ...
        if not self._xq_has_legal_move(nb, opp):
            ...
            games.setdefault(group_openid, {})[str(game_id)] = g

            save_json(XIANGQI_DATA_FILE, games)
            chess_cancel(group_openid, "xiangqi", game_id)
        ...
        if sc == ChatScene.GROUP:
            nxt = g["players"].get(g["turn"])
            if nxt and nxt != "AI":
                await qa_send(api, sc, group_openid, "轮到你走子啦～", mention_openid=nxt)



    async def _xiangqi_surrender(self, api, group_openid, member_openid, msg_id, game_id, scene=None):
    ...
    def has_active_session(self, group_openid: str) -> bool:
        ...
        gomoku_games = _chess_load_nested(GOMOKU_DATA_FILE)
        for gid, g in gomoku_games.get(group_openid, {}).items():
            if str(gid).isdigit() and g.get("status") == "playing":
                return True
        ...
        xiangqi_games = _chess_load_nested(XIANGQI_DATA_FILE)
        for gid, g in xiangqi_games.get(group_openid, {}).items():
            if str(gid).isdigit() and g.get("status") == "playing":
                return True
    ...
    def has_gomoku_session(self, group_openid: str) -> bool:
    ...
    def has_xiangqi_session(self, group_openid: str) -> bool:
    ...
    async def handle_command(self, api, content, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        ...
        if idiom_active:
            ...
            # 【全局指令隔离】任何人的全局指令先强制结束本轮猜成语，返回 False 让其他模块接手
            if self._is_global_command(text):
                await self._idiom_force_end(api, group_openid, msg_id, scene=scene,
                                            reason="收到其他指令「%s」" % text)
                return False

            # 群聊中仅发起者可作答/操作，非发起者的互动消息直接忽略，避免混淆
            if _scene(scene) == ChatScene.GROUP and member_openid != owner:
                return True

            # 任意有效互动重置 2 分钟超时计时
            qa_touch(group_openid, "idiom")
            ...
            # 【猜成语作答】按钮点入后：鲁棒处理某些 QQ 客户端可能直接发送占位符原文

            # 理想路径是 enter=False 填入输入框，玩家直接在后面接上答案发送，如“猜成语作答 画蛇添足”

            # 鲁棒1：如果玩家发了纯“猜成语作答”，提示怎么用

            if text == "猜成语作答":

                await send_text(api, _scene(scene), group_openid,

                    "✏️ 请在「猜成语作答」后面直接接上你的成语发送～\n例如：猜成语作答 守株待兔",
            ...
            # 仅当玩家以「猜成语作答」开头时才判定为答案，避免把闲聊/非回答内容当作答案

            if text.startswith("猜成语作答") and len(text) > 5:
                ...
                if guess == current_idiom:
                    ...
                    await self._idiom_correct(api, group_openid, member_openid, msg_id, scene=scene)

                else:

                    await self._idiom_wrong(api, group_openid, member_openid, guess, msg_id, scene=scene)

                return True

            # 非作答消息（闲聊、误触等）不判定为答案，直接忽略，避免反复误判错误

            return True



        # ========== 象棋优先路由（仅拦截本用户参与的进行中棋局，多局相互独立）==========

        # 象棋：仅拦截本用户参与的进行中棋局（多局相互独立）
        xiangqi_games = _chess_load_nested(XIANGQI_DATA_FILE)
        xq_gid, xq_game = _chess_find_by_player(xiangqi_games, group_openid, member_openid)

        if xq_gid is not None:

            if text == "结束象棋":

                await self._xiangqi_surrender(api, group_openid, member_openid, msg_id, xq_gid, scene=scene)
            ...
            if mv:
                ...
                await self._xiangqi_move(api, text, group_openid, member_openid, msg_id, xq_gid, scene=scene)
        ...
        if text == "猜成语":
            ...
            await self._idiom_start(api, group_openid, member_openid, msg_id, scene=scene)
        ...
        if text in ("认输", "结束棋局", "结束对局"):
            ...
            gomoku_games = _chess_load_nested(GOMOKU_DATA_FILE)
            gid, game = _chess_find_by_player(gomoku_games, group_openid, member_openid)

            if gid is not None and game.get("status") == "playing":
                ...
                await self._gomoku_surrender(api, group_openid, member_openid, msg_id, gid, scene=scene, member_nick=member_nick)
        ...
        # 结束象棋（兜底：未在进行中时由 _xiangqi_surrender 给出提示）

        if text == "结束象棋":

            xiangqi_games = _chess_load_nested(XIANGQI_DATA_FILE)
            xq_gid, _ = _chess_find_by_player(xiangqi_games, group_openid, member_openid)
            await self._xiangqi_surrender(api, group_openid, member_openid, msg_id, xq_gid, scene=scene)
        ...
        if text.startswith("下棋") or text.startswith("落子"):
            ...
            if gid is None:
                # 二人对战中第二位玩家通过首步落子加入
                gid, game = _chess_find_waiting(gomoku_games, group_openid, "gomoku")
            if gid is not None and game.get("status") == "playing":
                ...
                await self._gomoku_move(api, text, group_openid, member_openid, msg_id, gid, scene=scene, member_nick=member_nick)
...
# ============ 子功能分发方法（供细粒度插件壳调用）============
    ...
    async def handle_gomoku_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """五子棋：显式命令 + 认输 + 落子。返回 True 表示已处理。"""
        is_group = (scene == ChatScene.GROUP)
        # 显式命令
        if text == "五子棋":
            await self._gomoku_show_modes(api, group_openid, msg_id, scene=scene)
    ...
    async def handle_xiangqi_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """中国象棋：优先路由（进行中棋局走子）+ 显式命令 + 结束象棋。返回 True 表示已处理。"""
        # 优先路由：仅拦截本用户参与的进行中棋局（多局相互独立）
        xiangqi_games = _chess_load_nested(XIANGQI_DATA_FILE)
        xq_gid, xq_game = _chess_find_by_player(xiangqi_games, group_openid, member_openid)
        if xq_gid is not None:
            if text == "结束象棋":
        ...
        if text == "象棋排行":
            ...
            return True
        if text == "结束象棋":
            xq2_games = _chess_load_nested(XIANGQI_DATA_FILE)
            xq2_gid, _ = _chess_find_by_player(xq2_games, group_openid, member_openid)
            await self._xiangqi_surrender(api, group_openid, member_openid, msg_id, xq2_gid, scene=scene)
            return True
        return False

    async def handle_idiom_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """看图猜成语：优先路由（进行中会话拦截所有消息）+ 开始游戏。返回 True 表示已处理。"""
        idiom_games = load_json(IDIOM_GAME_DATA_FILE)
        idiom_active = (
            group_openid in idiom_games
            and idiom_games[group_openid].get("status") == "playing"
        ...
        if idiom_active:
            owner = qa_owner_openid(group_openid, "idiom")
            # 全局指令隔离：先强制结束本轮猜成语，返回 False 让其他模块接手
            if self._is_global_command(text):
                await self._idiom_force_end(api, group_openid, msg_id, scene=scene,
                                            reason="收到其他指令「%s」" % text)
                return False
            # 群聊中仅发起者可作答/操作，非发起者的互动消息直接忽略
            if _scene(scene) == ChatScene.GROUP and member_openid != owner:
                return True
            # 任意有效互动重置 2 分钟超时计时
            qa_touch(group_openid, "idiom")
    ...
    async def handle_qiuqian_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """观音灵签：求签。返回 True 表示已处理。"""
        if text == "求签":
            await self.send_qiuqian(api, group_openid, msg_id, scene=scene)
            return True
        return False

    async def handle_tarot_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """塔罗牌。返回 True 表示已处理。"""
        if text == "塔罗牌":
            await self.send_tarot(api, group_openid, msg_id, scene=scene)
            return True
    ...
    async def handle_daanzi_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """答案之书。返回 True 表示已处理。"""
        if text == "答案之书":
            await send_text(
                api, _scene(scene), group_openid,
    ...
    async def handle_horoscope_cmd(self, api, text, group_openid, member_openid, msg_id, scene=None, member_nick=None):
        """今日运势。返回 True 表示已处理。"""
        if text == "运势" or text == "今日运势" or text == "星座运势":
            await send_text(
                api, _scene(scene), group_openid,
...
def make_manager() -> GameManager:
    """返回娱乐系统共享管理器实例（五子棋/猜成语/象棋/占卜共用单例）。"""
    return _manager


def has_active_session(storage_id: str) -> bool:
    """检查该会话是否有进行中的五子棋、猜成语或象棋游戏（供 bot.py is_gaming 判断）。"""
    return _manager.has_active_session(storage_id)


def has_idiom_session(storage_id: str) -> bool:
    """检查该会话是否有进行中的猜成语游戏（供优先路由）。"""
