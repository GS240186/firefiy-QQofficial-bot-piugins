...
class ToolsManager:
    ...
    async def _query_weather_apihz(self, api, city, group_openid, msg_id, scene, days=1):
        """
        调用接口盒子(apihz.cn)中国气象局天气接口（主数据源）。
        接口: https://cn.apihz.cn/api/tianqi/tqyb.php
        参数: id(数字ID) / key(通讯密钥) / place(城市名) / [sheng(省,可选)] / [day=1~7 多天]
        返回: code=200 成功；含 nowinfo(实时) / alarm(预警) / weatherdayN(第N天预报)
        """
        try:
            from modules.config import APIHZ_TQ_ID, APIHZ_TQ_KEY
        except ImportError:
            APIHZ_TQ_ID, APIHZ_TQ_KEY = "", ""
        if not APIHZ_TQ_ID or not APIHZ_TQ_KEY:
            return False  # 未配置，交由小渡兜底
        ...
        if days > 1:
            params["day"] = days
        url = "https://cn.apihz.cn/api/tianqi/tqyb.php?" + urllib.parse.urlencode(params)
        headers = {"User-Agent": _UA_PC}
        ...
        try:
            ...
            logger.error("接口盒子天气请求异常: %s" % e)
            return False

        if data.get("code") != 200:
            # 400 多为地址不被支持（如区级），返回 False 走兜底
            return False

        name = data.get("name") or data.get("shi") or city
        ...
        # 实时数据
        now = data.get("nowinfo") or {}
        now_temp = now.get("temperature", "")
        now_hum = now.get("humidity", "")
        now_wind = now.get("windDirection", "")
        now_scale = now.get("windScale", "")
        now_feel = now.get("feelst", "")

        lines = ["🌤 天气查询", "━━━━━━━━━━", "📍 %s" % name]
        # 实时
        if now_temp != "":
            lines.append("🌡 实时温度：%s℃" % now_temp)
            if now_feel != "":
                lines.append("🤚 体感温度：%s℃" % now_feel)
        # 今日区间
        day_desc = weather1
        if weather2 and weather2 != weather1:
            day_desc = "%s 转 %s" % (weather1, weather2)
        temp_desc = ""
        if wd1 != "":
            temp_desc = wd1
            if wd2 != "" and str(wd2) != str(wd1):
                temp_desc = "%s ~ %s" % (wd2, wd1)
        lines.append("🌀 今日天气：%s" % day_desc)
        if temp_desc:
            lines.append("🌡 温度区间：%s℃" % temp_desc)
        if now_hum != "":
            lines.append("💧 湿度：%s%%" % now_hum)
        if now_wind:
            lines.append("🍃 风力：%s %s" % (now_wind, now_scale))
        if uptime:
            lines.append("🕐 更新：%s" % uptime)

        # 多天预报
        if days > 1:
            lines.append("━━━━━━━━━━")
            lines.append("📅 未来%d天预报：" % days)
            for n in range(2, days + 1):
                dk = data.get("weatherday%d" % n)
                if not dk:
                    continue
                d_w1 = dk.get("weather1", "")
                d_w2 = dk.get("weather2", "")
                d_desc = d_w1 if (not d_w2 or d_w2 == d_w1) else "%s 转 %s" % (d_w1, d_w2)
                d_temp = ""
                if dk.get("wd1") != "":
                    d_temp = "%s" % dk.get("wd2")
                    if str(dk.get("wd2")) != str(dk.get("wd1")):
                        d_temp = "%s ~ %s" % (dk.get("wd2"), dk.get("wd1"))
                lines.append("· %s %s：%s%s" % (
                    dk.get("monthDay", ""), dk.get("weekday", ""),
                    d_desc,
                    ("  %s℃" % d_temp) if d_temp else "",
                ))
    ...
    async def _call_apihz(self, path, params=None):
        """调用接口盒子，返回 (ok, data_dict)。失败返回 (False, None)。"""
        if not APIHZ_TQ_ID or not APIHZ_TQ_KEY:
            return False, None
        url_params = {"id": APIHZ_TQ_ID, "key": APIHZ_TQ_KEY}
        if params:
            url_params.update(params)
        url = "https://cn.apihz.cn%s?%s" % (path, urllib.parse.urlencode(url_params))
        try:
            req = urllib.request.Request(url, headers={"User-Agent": _UA_PC}, method="GET")
            with urllib.request.urlopen(req, timeout=12) as resp:
                raw = resp.read().decode("utf-8")
            data = json.loads(raw)
            return True, data
        except Exception as e:
            logger.error("[工具] apihz 请求异常 %s: %s" % (path, e))
            return False, None
    ...
    async def _query_navigation(self, api, args, group_openid, msg_id, scene):
        """
        导航规划：基于起点/终点经纬度。
        接口：/api/jiaotong/daohang.php
        参数：starlon, starlat, endlon, endlat
        ...
        if len(parts) < 2:
            await send_text(
                api, scene, group_openid,
                "🧭 导航规划\n━━━━━━━━━━\n用法：导航 起点经度,起点纬度 终点经度,终点纬度\n示例：导航 104.06,30.67 104.07,30.68\n提示：需输入经纬度坐标（可从地图 App 复制）",
                msg_id=msg_id,
            )
            return
        try:
            start = [float(x.strip()) for x in parts[0].split(",")]
            end = [float(x.strip()) for x in parts[1].split(",")]
        ...
        ok, data = await self._call_apihz(
            "/api/jiaotong/daohang.php",
            {"starlon": start[0], "starlat": start[1], "endlon": end[0], "endlat": end[1]},
        )
        if not ok or not data or data.get("code") != 200:
            err = data.get("msg", "查询失败") if isinstance(data, dict) else "查询失败"
            await send_text(api, scene, group_openid, "🧭 导航规划\n━━━━━━━━━━\n%s" % err, msg_id=msg_id)
            return

        summary = data.get("route_summary") or {}
        steps = data.get("simplified_steps") or data.get("detailed_steps") or []
        lines = ["🧭 导航规划", "━━━━━━━━━━"]
        lines.append("起点：%s" % summary.get("origin", "%.5f,%.5f" % (start[0], start[1])))
        lines.append("终点：%s" % summary.get("destination", "%.5f,%.5f" % (end[0], end[1])))
        dist = summary.get("total_distance_km")
        dur = summary.get("total_duration_sec")
        if dist:
            lines.append("总距离：约 %.2f 公里" % float(dist))
        if dur:
            lines.append("预计时间：约 %d 分钟" % (int(float(dur)) // 60))
        if steps:
            lines.append("━━━━━━━━━━")
            lines.append("📍 路线概览：")
    ...
    async def _query_tourism(self, api, city, group_openid, msg_id, scene):
        """
        旅游景点查询。
        接口：/api/fun/jingdian.php
        参数：words=城市名
        """
        ok, data = await self._call_apihz("/api/fun/jingdian.php", {"words": city})
        if not ok or not data or data.get("code") != 200:
            await send_text(api, scene, group_openid, "🏞️ 旅游查询\n━━━━━━━━━━\n未找到「%s」相关景点信息，换个城市试试～" % city, msg_id=msg_id)
            return
        items = data.get("datas") or []
        if not items:
            await send_text(api, scene, group_openid, "🏞️ 旅游查询\n━━━━━━━━━━\n未找到「%s」相关景点信息，换个城市试试～" % city, msg_id=msg_id)
            return
        lines = ["🏞️ %s 旅游景点" % (data.get("city") or city), "━━━━━━━━━━"]
...
单词 cancel""",
    ...
    async def handle_command(self, api, content, group_openid, member_openid, msg_id, scene=None):
        ...
        if text.startswith("天气 ") or text.startswith("天气\u3000"):
            city = text[2:].strip()
            if city:
                # 解析末尾可选「天数」参数（如「天气 绵阳 3」）
                days = 1
                parts = city.split()
                if len(parts) >= 2 and parts[-1].isdigit():
                    days = int(parts[-1])
                    city = " ".join(parts[:-1]).strip()
                # 优先接口盒子（支持多天/实时/预警）；失败回退小渡
                ok = await self._query_weather_apihz(api, city, group_openid, msg_id, scene, days)
                if not ok:
        ...
        if text.startswith("导航 ") or text.startswith("导航\u3000"):
            args = text[2:].strip()
            await self._query_navigation(api, args, group_openid, msg_id, scene)
            return True
        ...
        if text.startswith("旅游 ") or text.startswith("旅游\u3000"):
            city = text[2:].strip()
            if city:
                await self._query_tourism(api, city, group_openid, msg_id, scene)
                return True
            await send_text(
                api, scene, group_openid,
                "🏞️ 旅游查询\n"
