...
"""
图片系统模块
基于小小API 多个端点 + 接口盒子 apihz.cn，随机返回高清图片：
- 二次元（acg）：精致二次元插画
- 风景（wallpaper）：震撼风景壁纸
- 随机壁纸（双源随机：小小API wallpaper / 接口盒子 apihzimgbz）
- 原神cos（yscos）：原神cosplay 图片
- 原神（ys）：原神游戏图
- 小姐姐（meinvpic）：高清美图

鉴权：小小API 支持无 Key 调用；填写 RANDOMPIC_API_KEY（=小小API统一密钥）可提高稳定性与额度，
以 Authorization: Bearer <key> 头携带。接口盒子复用天气同一账户 id/key。
"""
...
class ImageManager:
    ...
    async def _fetch_image_url(self, category: str):
        """
        向对应接口请求一张随机图片地址（按分类映射到独立端点/参数）。
        「随机壁纸」双源随机切换：小小API wallpaper / 接口盒子 apihzimgbz。
        返回图片直链字符串，或 None（失败/无图）。
        """
        # 随机壁纸双源随机切换
        if category == "随机壁纸":
            sources = []
            if RANDOMBIZHI_API_URL:
                sources.append("xxapi")
            if APIHZ_TQ_ID and APIHZ_TQ_KEY:
                sources.append("apihz")
            if not sources:
                logger.warning("随机壁纸未配置任何可用图片来源")
                return None
            source = random.choice(sources)
            if source == "apihz":
                return await self._fetch_apihz_wallpaper_url()
            # 否则走下方默认逻辑（小小API）
    ...
    async def _fetch_apihz_wallpaper_url(self):
        """调用接口盒子 apihzimgbz 随机壁纸接口，返回图片直链或 None。"""
        imgtype = random.choice(["1", "2"])
        url = (
            "https://cn.apihz.cn/api/img/apihzimgbz.php?id=%s&key=%s&type=1&imgtype=%s"
            % (urllib.parse.quote(str(APIHZ_TQ_ID)), urllib.parse.quote(APIHZ_TQ_KEY), imgtype)
        )
        try:
            data = await http_get(url, params=None, headers={"User-Agent": "Mozilla/5.0"}, timeout=12)
        except Exception as e:
            logger.warning("随机壁纸接口盒子请求异常: %s" % e)
            return None
        if isinstance(data, dict) and data.get("code") == 200:
            pic = data.get("msg") or data.get("data")
            if isinstance(pic, str) and pic.startswith("http"):
                return pic
        logger.warning("随机壁纸接口盒子返回异常: %r" % (data if isinstance(data, dict) else type(data)))
        return None
