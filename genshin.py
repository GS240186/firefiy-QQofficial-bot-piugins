...
# 极限面板: 仿云崽 `#角色名极限面板` / `角色名极限面板`
_MAX_PANEL_RE = re.compile(
    r"^#\s*(?:原神|ys|genshin)?\s*([一-龥A-Za-z·\u3000]{2,8})\s*极限面板\s*$"
)
...
async def _send_max_panel(ctx, char_name):
    """渲染并发送理论极限面板图 (合成 MAX 数据, 走 miao 渲染管线).

    不需要绑定 UID. 失败时回退文本说明原因.
    """
    try:
        from lib.genshin_panel_miao.max_panel import build_max_panel_data
        from lib.genshin_panel_miao.render import render_panel
        data = build_max_panel_data(char_name)
        if "error" in data:
            await ctx.reply("❌ " + data["error"])
            return True
        png = await render_panel(data)
        if png:
            try:
                from modules.common import send_local_image_for_scene
                await send_local_image_for_scene(
                    ctx.api, ctx.scene, ctx.target_id, png,
                    content="✨ %s 理论极限面板 (MAX)" % data.get("char_name", char_name),
                )
                return True
            except Exception as e:
                await ctx.reply("（极限面板图发送失败: %s）" % e)
                return True
        await ctx.reply("（极限面板图渲染失败，请查看 bot 控制台 stderr 输出）")
        return True
    except Exception as e:
        await ctx.reply("（极限面板图处理异常: %s）" % e)
        return True
...
async def handle(ctx) -> bool:
    ...
    #    7a) 极限面板: 胡桃极限面板 / #胡桃极限面板 (无需绑定 UID)
    max_m = _MAX_PANEL_RE.match(content)
    if max_m:
        raw_name = max_m.group(1).strip()
        char_name = _resolve_alias(raw_name)
        if char_name is None:
            await ctx.reply("⚠️ 无法识别角色名「%s」。请输入标准名 (胡桃/芙宁娜/雷电将军) 或别名 (雷神/水神/芙芙/护法夜叉)。" % raw_name)
            return True
        await _send_max_panel(ctx, char_name)
        return True
